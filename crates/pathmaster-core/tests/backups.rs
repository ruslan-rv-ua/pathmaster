...
//! they come in (spec §8, FR-backup-ui; ADR-0006).
//!
...
/// One Snapshot file, as the tab builds it: a name, and what reading it gave.
fn file(name: SnapshotName, decoded: Decoded) -> SnapshotFile {
    backups::newest_first([(name, decoded)])
        .pop()
        .expect("one name is one file")
}
...
fn a_row_dates_a_snapshot_the_way_a_person_reads_a_date() {
    ...
    let file = file(name.clone(), valid(&name, present(&[r"C:\bin"])));

    assert_eq!(file.taken(), "2026-08-19 14:32:07");
}
...
fn a_row_takes_its_scope_from_the_name_so_a_corrupted_file_still_has_one() {
    ...
    let file = file(name, Decoded::Corrupted);

    assert_eq!(file.scope(), Scope::System);
    assert_eq!(file.taken(), "2026-08-19 14:32:07");
}
...
fn a_corrupted_snapshot_has_nothing_to_restore() {
    ...
    assert_eq!(file(name, Decoded::Corrupted).restores(), None);
}
...
fn a_row_restores_the_entries_and_the_value_type_the_snapshot_captured() {
    ...
    let (entries, value_type) = file.restores().expect("a valid Snapshot restores");
...
fn a_snapshot_of_an_absent_scope_restores_an_empty_working_copy() {
    ...
    let name = name(Scope::System, 7);
    let file = file(name.clone(), valid(&name, Captured::Absent));

    let (entries, value_type) = file.restores().expect("an Absent Snapshot restores");
...
fn the_list_shows_the_newest_snapshot_first() {
    ...
    let files = backups::newest_first([
...
fn two_snapshots_of_one_second_come_back_in_the_order_their_suffixes_give_them() {
    ...
    assert_eq!(
        files
            .iter()
            .map(|file| file.restores().expect("a valid Snapshot").0[0].clone())
            .collect::<Vec<String>>(),
        ["last", "second", "first"],
    );
}
