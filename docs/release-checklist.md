...
### Where the run lands (`--tab`)

Step 1 covers the tab a plain launch opens on. `--tab` is the only other way to choose one from
outside the application, and it had no step of its own in any earlier release.

| # | Step | Expected speech | ✓ |
|---|------|-----------------|---|
| 90 | Launch with `--tab system`, and again with `--tab backups` | Each opens on that tab and lands the focus on **its** list's first row, read on arrival — never the tab strip, and never the User list. On Backups the row reads as step 19 spells it; on System it reads without its Status, no pass having completed. `NVDA+Tab` answers with the row both times | |
...
| C9 | Stage C8 again from the **System tab**, answer [Discard and Restart], accept the UAC prompt | The original instance exits — its log ends `INFO  shutdown: clean` — and the elevated one opens **on the System tab**, titled "Administrator: PathMaster", and lands the focus on that list's **first row**, read without its Status — the landing of step 1, on the tab the relaunch chose; `NVDA+Tab` answers with the row, not the tab strip. The registry value is unchanged, because Discard saved nothing | |
...
| F3 | Push that commit to `develop`, let push CI go green, then cut the release with `git flow release start` / `finish`, which merges into `main` and tags it there — **then check the tag's name before pushing it** | The release workflow re-runs the whole gate itself — `cargo fmt --check`, `cargo test`, `cargo clippy` — before it builds anything, and only then the three-way version gate. **The tag has to land on `main`**, because a hotfix branches from `main`: tag `develop` instead and the first urgent fix has an empty branch to start from. **And it has to be named `v<version>`**, which `finish` now does on its own: `git-flow-next` names the tag from the branch and takes its prefix from `gitflow.branch.<type>.tagprefix`, set to `v` here for `release` and `hotfix`. The key is real but unadvertised — `git flow config edit topic release` knows only `--tag=true \| false`, which is what made v0.2.0 conclude the setting did not exist and rename its tag by hand. **It lives in `.git/config`, which is not committed**: a fresh clone, or another machine, has it unset, and `finish` then tags a bare `<version>` where `release.yml` waits on `v*`. A tag without the `v` starts nothing — no build, no release page, and no red anything; you find out by waiting. So read the tag's name before the push, and if it came out bare, set the key and fix the tag: `git tag -d <version>` then `git tag -a v<version> -m "PathMaster v<version>" <commit>`. Push CI watches `develop` while the tag lands on `main`, so the tagged commit may be one push CI never saw — going through `develop` first gets you the answer sooner, it is not the only thing standing between a release and untested code | |
