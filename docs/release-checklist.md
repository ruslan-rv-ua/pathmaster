...
| F3 | Push that commit to `develop`, let push CI go green, then tag the commit the release is cut from | The release workflow re-runs the whole gate itself — `cargo fmt --check`, `cargo test`, `cargo clippy` — before it builds anything, and only then the three-way version gate. Push CI watches `develop` while the tag lands on `main`, so the tagged commit may be one push CI never saw: going through `develop` first is an ordering habit that gets you the answer sooner, not the only thing standing between a release and untested code | |
